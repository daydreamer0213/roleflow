...
function messageDiscoveryClientScript(scriptState) {
  return `<script>(function(){
    ...
    const runTransitions=async()=>{if(transitionRunning)return;transitionRunning=true;if(sourceFilter)sourceFilter.disabled=true;while(pendingTransition){let next=pendingTransition;pendingTransition=null;const current=messagePanels.find(panel=>panel.dataset.messageDetailPanel===selectedKey);const fields=Array.from(current?.querySelectorAll("[data-draft-text]")||[]).filter(field=>!field.disabled);try{await Promise.all(fields.map(saveDraft));}catch{pendingTransition=null;feedback.textContent="当前草稿未能保存，已保留当前消息，请稍后重试。";applySourceFilter(selectedKey,false);break;}if(pendingTransition){next=pendingTransition;pendingTransition=null;}if(next.source)selectedSource=next.source;try{localStorage.setItem("message-source-"+initial.profileId,selectedSource);}catch{}applySourceFilter(next.key||selectedKey);}transitionRunning=false;if(sourceFilter)sourceFilter.disabled=false;if(pendingTransition)runTransitions();};
