...
function renderPlanSummary(vm) {
  ...
  return `<section class="card" aria-labelledby="plan-summary-title"><div class="card-head"><div><p class="section-label">本地筛选方案</p><h2 id="plan-summary-title">${escapeHtml(plan.name || "未命名方案")}</h2></div><a class="button secondary" href="#plan-settings">调整方案</a></div><div class="card-body"><div class="plan-scope"><div><strong>平台已继承</strong><p>${escapeHtml(profile.bossFilter?.summary || "沿用 BOSS 搜索页当前条件")}</p></div><div><strong>本地再筛选</strong><p>${escapeHtml(localFilterSummary(plan))}</p></div></div><dl class="definition-grid"><div><dt>关键词</dt><dd>${escapeHtml((plan.keywords || []).map((item) => item.word || item).filter(Boolean).join("、") || "待确认")}</dd></div><div><dt>工作经验</dt><dd>${escapeHtml((plan.experience || []).join("、") || "沿用平台条件")}</dd></div><div><dt>薪资策略</dt><dd>${escapeHtml(`${plan.salary?.minK ?? ""}${plan.salary?.maxK ? `–${plan.salary.maxK}K` : ""}` || "仅排序，不硬排除")}</dd></div><div><dt>工作节奏</dt><dd>${escapeHtml(plan.workSchedulePreference === "no_preference" ? "不作为排序依据" : "优先双休，其他仍保留")}</dd></div></dl><details class="today-secondary"><summary>查看版本与反馈</summary>${renderVersionDiff(profile.versionDiff)}${renderFeedback(profile.feedback)}</details><div class="button-row"><a class="button quiet" href="/profile?profileId=${escapeAttr(profile.id)}">查看候选人画像</a><a class="button quiet" href="/resumes?profileId=${escapeAttr(profile.id)}">管理简历版本</a></div></div></section>`;
}
