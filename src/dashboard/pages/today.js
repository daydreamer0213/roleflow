...
function renderClientScripts(runState, includeBrowserReadiness) {
  ...
  return `${readiness}<script>(function(){const form=document.getElementById('plan-form');const note=document.getElementById('plan-dirty-note');if(!form)return;function syncAcquisitionPanels(){const mode=form.querySelector('input[name=acquisitionMode]:checked')?.value||'inherited';form.querySelectorAll('[data-acquisition-panel]').forEach(function(panel){const visible=panel.dataset.acquisitionPanel===mode;panel.hidden=!visible;panel.setAttribute('aria-hidden',visible?'false':'true')})}syncAcquisitionPanels();form.querySelectorAll('input[name=acquisitionMode]').forEach(function(input){input.addEventListener('change',syncAcquisitionPanels)});form.addEventListener('input',function(){document.querySelectorAll('[data-scan-button]').forEach(function(button){button.disabled=true});if(note)note.hidden=false});}());</script>${runState === "running" ? `<script>setTimeout(()=>location.reload(),2500)</script>` : ""}`;
}
