...
function renderProgressTracks(progress = {}) {
  ...
  return `<section class="workflow-tracks" aria-labelledby="workflow-tracks-title"><div class="workflow-tracks-heading"><div><h2 id="workflow-tracks-title">本轮逐项进度</h2><p aria-live="polite" data-current-activity>${escapeHtml(progress.currentActivityLabel || "")}</p></div></div>${tracks}${jobs ? `<section class="workflow-task-list" aria-labelledby="workflow-task-list-title"><h3 id="workflow-task-list-title">逐岗位分析</h3>${jobs}</section>` : ""}</section>`;
}
...
function renderProgress(progress) {
  ...
  return `<section class="workflow-live" data-workflow-panel data-progress-revision="${number(progress.revision)}"><div class="workflow-live-head"><div><p class="workflow-stage" data-stage-label>第 ${number(progress.stageIndex)} 阶段 / 共 ${number(progress.stageCount)} 阶段</p><h2 data-stage-name>${escapeHtml(progress.stage)}</h2></div><p class="workflow-model">${escapeHtml(progress.modelLabel)}</p></div><div class="workflow-analysis-grid" aria-label="采集与分析数量">${stat("搜索目标", scan.total, "data-scan-target-total")}${stat("目标完成", scan.completed, "data-scan-target-completed")}${stat("目标待完成", scan.pending, "data-scan-target-pending")}${stat("已采集岗位", details.collected, "data-detail-collected")}${stat("已读详情", details.read, "data-detail-read")}${stat("待读详情", details.pending, "data-detail-pending")}${stat("待分析", analysis.remaining, "data-analysis-remaining")}${stat("分析成功", analysis.succeeded, "data-analysis-succeeded")}${stat("分析失败", analysis.failed, "data-analysis-failed")}</div><div class="workflow-live-meta"><section class="workflow-live-card"><h3>超时保护</h3><p data-analysis-timeouts>当前恢复周期最终超时 ${number(analysis.circuitTimeoutJobs)} / ${number(analysis.timeoutPauseThreshold)} · 本轮累计超时 ${number(analysis.lifetimeTimeoutJobs)}</p><p data-eta>${escapeHtml(progress.etaLabel)}</p></section><section class="workflow-live-card"><h3>最近活动</h3><p data-recent-activity>${escapeHtml(progress.recentActivityLabel)}</p></section></div><span hidden data-analysis-total>${number(analysis.total)}</span><span hidden data-analysis-running>${number(analysis.running)}</span><span hidden data-analysis-retry-pending>${number(analysis.retryPending)}</span><span hidden data-analysis-detail-required>${number(analysis.detailRequired)}</span><p class="workflow-scan-wait" data-scan-wait${progress.scanWaitLabel ? "" : " hidden"}>${escapeHtml(progress.scanWaitLabel)}</p><p class="workflow-stale" data-workflow-stale hidden>任务可能失去活动，请先检查诊断信息，不要重复启动。</p></section>`;
}
...
function renderPhase(phase = {}) {
  ...
  if (phase.kind === "interrupted") return `<section class="workflow-phase"><div class="workflow-actions">${phase.communicationHref ? `<a class="button-link" data-workflow-primary="true" href="${escapeAttr(phase.communicationHref)}">${escapeHtml(phase.communication?.detailsLabel || "检查沟通中断项")}</a>` : renderResume(phase.resume)}</div><div class="workflow-alert"><strong>本轮已中断</strong><p>${phase.communicationHref ? `${escapeHtml(communicationErrorLabel(phase.errorCode))}<br>` : ""}${escapeHtml(phase.errorCode)} · ${escapeHtml(phase.errorMessage)}</p></div></section>`;
