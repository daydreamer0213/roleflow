...
function renderWorkflowPage(vm = {}) {
  ...
  const primaryContent = actionFirst
    ? `${command}${phase}${overview}`
    : `${overview}${command}${phase}`;
  ...
  ${renderHeader(vm)}${error}${primaryContent}<details class="workflow-technical"><summary>技术明细</summary>${renderRunMetrics(vm.header)}${renderScope(vm.scope)}${vm.health?.report?.status ? renderWorkflowHealthPanel(vm.health.report) : ""}${renderProgress(vm.progress)}</details>
...
function renderPrimary(vm = {}) {
  ...
  return `<section class="workflow-primary" role="region" aria-labelledby="workflow-primary-title"><h2 id="workflow-primary-title">本轮概览</h2><dl class="workflow-primary-grid">${primaryField("当前阶段", overview.currentPhase, false, "phase")}${primaryField("整体进度", overview.overallProgress, false, "progress")}${primaryField("采集进度", overview.acquisitionProgress, false, "acquisition")}${primaryField("完整 JD", overview.jdProgress, false, "jd")}${primaryField("可用推荐", overview.usableRecommendations, false, "recommendations")}${primaryField("剩余工作", overview.remainingWork, false, "remaining")}${primaryField("预计继续时间", overview.estimatedContinuation, false, "eta")}${primaryField("暂停/阻塞原因", blockerDetail, true, "blocker")}${primaryField("下一步", overview.nextAction, false, "next-action")}</dl></section>`;
}
...
function renderScope(scope = {}) {
  ...
  return `<section class="workflow-scope" data-workflow-diagnostics><h2>采集与筛选明细</h2><strong>筛选来源：BOSS 当前页面</strong><ul>${(scope.filters || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("") || "<li>使用平台默认筛选</li>"}</ul><p>范围：${escapeHtml(scope.scopeKey)} · 关键词来源：Search Plan #${escapeHtml(scope.sourcePlanId)}</p><p>本轮实际关键词：${escapeHtml((scope.actualKeywords || []).join("、") || "无")}</p><p class="hint">方案候选词：${number(scope.candidateKeywordCount)} 个；本轮只展示已冻结执行的关键词。</p>${scope.unresolved?.length ? `<p class="workflow-alert">未解析平台筛选：${escapeHtml(scope.unresolved.join("、"))}；采集 URL 已保留这些条件，本地不会猜值。</p>` : ""}<p class="hint">修改 BOSS 筛选会创建新的统计范围；本轮恢复仍使用当前冻结范围。</p></section>`;
}
...
function renderProgress(progress) {
  ...
  return `<section class="workflow-live" data-workflow-panel data-progress-revision="${number(progress.revision)}"><div class="workflow-live-head"><div aria-live="polite"><p class="workflow-stage" data-stage-label>第 ${number(progress.stageIndex)} 阶段 / 共 ${number(progress.stageCount)} 阶段</p><h2 data-stage-name>${escapeHtml(progress.stage)}</h2></div><p class="workflow-model">${escapeHtml(progress.modelLabel)}</p></div><progress class="workflow-meter" data-analysis-progress aria-label="岗位分析完成进度" max="${number(progress.meter?.max) || 1}" value="${number(progress.meter?.value)}"></progress><div class="workflow-analysis-grid" aria-label="采集与分析数量">${stat("搜索目标", scan.total, "data-scan-target-total")}${stat("目标完成", scan.completed, "data-scan-target-completed")}${stat("目标待完成", scan.pending, "data-scan-target-pending")}${stat("已采集岗位", details.collected, "data-detail-collected")}${stat("已读详情", details.read, "data-detail-read")}${stat("待读详情", details.pending, "data-detail-pending")}${stat("待分析", analysis.remaining, "data-analysis-remaining")}${stat("分析成功", analysis.succeeded, "data-analysis-succeeded")}${stat("分析失败", analysis.failed, "data-analysis-failed")}</div><div class="workflow-live-meta"><section class="workflow-live-card"><h3>超时保护</h3><p data-analysis-timeouts>当前恢复周期最终超时 ${number(analysis.circuitTimeoutJobs)} / ${number(analysis.timeoutPauseThreshold)} · 本轮累计超时 ${number(analysis.lifetimeTimeoutJobs)}</p><p data-eta>${escapeHtml(progress.etaLabel)}</p></section><section class="workflow-live-card"><h3>最近活动</h3><p data-recent-activity aria-live="polite">${escapeHtml(progress.recentActivityLabel)}</p></section></div><span hidden data-analysis-total>${number(analysis.total)}</span><span hidden data-analysis-running>${number(analysis.running)}</span><span hidden data-analysis-retry-pending>${number(analysis.retryPending)}</span><span hidden data-analysis-detail-required>${number(analysis.detailRequired)}</span><p class="workflow-scan-wait" data-scan-wait${progress.scanWaitLabel ? "" : " hidden"}>${escapeHtml(progress.scanWaitLabel)}</p><p class="workflow-stale" data-workflow-stale hidden>任务可能失去活动，请先检查诊断信息，不要重复启动。</p></section>`;
}
...
function renderPhase(phase = {}) {
  ...
  if (phase.kind === "interrupted") return `<section class="workflow-phase"><div class="workflow-actions">${phase.communicationHref ? `<a class="button-link" data-workflow-primary="true" href="${escapeAttr(phase.communicationHref)}">${escapeHtml(phase.communication?.detailsLabel || "检查沟通中断项")}</a>` : renderResume(phase.resume)}</div><div class="workflow-alert"><strong>本轮已中断</strong><p>${escapeHtml(phase.errorCode)} · ${escapeHtml(phase.errorMessage)}</p></div></section>`;
