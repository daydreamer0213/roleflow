...
const PROGRESS_ACTIONS = Object.freeze({
  reply_confirmed_sent: { stage: "waiting_reply", eventType: "reply_confirmed_sent", summary: "用户确认已手动发送", nextAction: "等待招聘方回复" },
  mark_needs_user_action: { stage: "needs_user_action", eventType: "manual_review_requested", summary: "用户标记需要处理", nextAction: "请用户确认下一步" },
  mark_interview_invited: { stage: "interview_invited", eventType: "interview_invited", summary: "用户确认收到面试邀约", nextAction: "请用户决定是否接受并安排时间" },
  mark_interview_scheduled: { stage: "interview_scheduled", eventType: "interview_scheduled", summary: "", nextAction: "按已确认时间参加面试" },
  mark_resume_submitted: { stage: "resume_submitted", eventType: "resume_submitted", summary: "用户确认已投递简历", nextAction: "等待招聘方后续反馈" },
  close_opportunity: { stage: "closed", eventType: "closed_by_user", summary: "用户关闭机会", nextAction: "" },
...
function renderCompactJob(job, filters) {
  ...
  return html
    .replace(/<form class="quick-actions" method="post" action="\/api\/mark[^"]*">.*?<\/form>/, "")
    .replace(/<form class="detail-actions" method="post" action="\/api\/mark[^"]*">.*?<\/form>/, "")
    .replace("</details></article>", `</details>${renderProgressPanel(job.progressCard)}</article>`);
}
...
function renderCompactJobBase(job, filters) {
  ...
  const status = job.progressCard ? ` · ${progressStageLabel(job.progressCard.stage)}` : job.applicationStatus ? ` · ${statusLabel(job)}` : "";
