  ...
  try {
    ...
    const conflicts = db.prepare(`SELECT communication_batch_items.id FROM communication_batch_items
      ...
      WHERE communication_batch_items.job_id = ?
        AND communication_batch_items.status IN ('click_dispatched', 'ambiguous')
      LIMIT 1`);
...
function transitionCommunicationItem(db, input = {}) {
  ...
  if (TERMINAL_ITEM_STATUSES.has(expectedStatus)
    && !(expectedStatus === "ambiguous" && ["succeeded", "stopped"].includes(status))) {
  ...
  if (dispatching && current.clickCount >= 1) {
  ...
  return getCommunicationBatchItem(db, itemId);
}
...
function resolveAmbiguousCommunicationItem(db, input = {}) {
  ...
  return transitionCommunicationItem(db, { ...input, expectedStatus: "ambiguous", status });
}
