...
function createCommunicationBatch({ db, input = {}, deps = {} }) {
  ...
  const requestedSite = input.site || "boss";
  const batch = createImmutableCommunicationBatch(db, {
    ...
    policySnapshot: { calibration: communicationCalibrationStatus(requestedSite) }
  });
...
function validateCommunicationControl({ db, input = {}, deps = {} }) {
  ...
  if (action === "start" || action === "resume") {
    ...
    const calibration = (deps.communicationCalibrationReader || communicationCalibrationStatus)(batch.site);
    ...
    const singleItem = singleItemAction
      ...
      : null;
    assertCommunicationExecutionEnabled(batch.site);
    assertCommunicationRuntimeAvailable(db, { site: batch.site });
...
function getCommunicationStatus({ db, batchId, deps = {} }) {
  ...
  return {
    ...
    quota: communicationQuotaSnapshot(db, { site: batch.site }),
    calibration: communicationCalibrationStatus(batch.site),
    runtimeBlock: communicationRuntimeBlock(db, { site: batch.site })
  };
}
