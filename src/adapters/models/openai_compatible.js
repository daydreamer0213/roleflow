...
class OpenAICompatibleAdapter {
  ...
  async matchJob(input) {
    const sparsePrompt = [
      ...
      "Match by meaning, not exact wording. When an R* requirement is a broad umbrella capability, a narrower concrete candidate fact may be matched if that fact is a direct instance of the required work; use transferable only when it proves the same underlying capability in a different domain or tool. Cite the concrete fact. Do not reverse this relation: broad or adjacent experience does not prove a named platform, specialist workflow, stack, or business system absent from the candidate facts.",
      "This is a semantic-level example, not an occupation rule: for broad “AI 工具实践”, concrete Agent/RAG/Dify workflow delivery is a direct instance; for broad “AI 代码调试”, concrete logging, tests, mock, exception tracing, or API debugging is a direct instance. Wording difference alone is not a reason to omit the row. These facts still do not prove an unmentioned image-generation/visual workflow, named Agent platform, data warehouse, big-data framework, or ERP integration.",
