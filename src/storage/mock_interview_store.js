...
function ownedSessionRow(db, profileId, sessionId) {
  return db.prepare("SELECT * FROM mock_interview_sessions WHERE id = ? AND profile_id = ?")
    .get(positiveId(sessionId, "sessionId"), positiveId(profileId, "profileId"));
}
...
function createMockInterviewSession(db, input = {}) {
  ...
  const ownedJob = db.prepare(`SELECT jobs.id FROM jobs
    WHERE jobs.id = ? AND EXISTS (
      ...
      WHERE o.job_id = jobs.id AND b.profile_id = ?
    )`).get(jobId, profileId);
  if (!ownedJob) throw storageError("MOCK_INTERVIEW_JOB_NOT_FOUND", "岗位不存在或不属于当前候选人");
  const ownedResume = db.prepare("SELECT id FROM candidate_resume_versions WHERE id = ? AND profile_id = ?")
    .get(resumeVersionId, profileId);
  if (!ownedResume) throw storageError("MOCK_INTERVIEW_RESUME_NOT_FOUND", "简历不存在或不属于当前候选人");
  ...
  return getMockInterviewSession(db, { profileId, sessionId: Number(result.lastInsertRowid) });
}

function getMockInterviewSession(db, { profileId, sessionId }) {
  return sessionRow(db, ownedSessionRow(db, profileId, sessionId));
}

function listMockInterviewSessions(db, profileId, limit = 30) {
...
function appendMockInterviewQuestion(db, input = {}) {
  ...
  const questionText = requiredText(question.text, "问题", 4_000);
  const questionFocus = requiredText(question.focus, "问题重点", 120);
  return immediateTransaction(db, () => {
    const session = ownedSessionRow(db, profileId, sessionId);
    ...
    const basedOnTurnNumber = question.basedOnTurnNumber == null || question.basedOnTurnNumber === ""
      ? null
      : Number(question.basedOnTurnNumber);
    const expectedBasedOn = previous ? Number(previous.turn_number) : null;
    if (basedOnTurnNumber !== expectedBasedOn) throw new Error("下一题必须承接上一题");
    const now = nowIso();
    const result = db.prepare(`INSERT INTO mock_interview_turns(
      ...
      sessionId, turnNumber, questionText, questionFocus, basedOnTurnNumber, now, now
    );
...
function completeMockInterviewSession(db, input = {}) {
  ...
  return immediateTransaction(db, () => {
    const row = ownedSessionRow(db, profileId, sessionId);
