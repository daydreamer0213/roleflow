...
4. 运行 `npm.cmd test`。当前 42 项离线检查不访问 BOSS。
...
- `PROGRESS_STAGE_TRANSITION_INVALID`：进展阶段不能按当前动作直接切换；使用人工纠错并填写原因。
- `PROGRESS_INTERVIEW_SUMMARY_REQUIRED`：标记已安排面试前，需要填写用户确认的短安排摘要。
...
求职进展卡按 `profile_id + job_id` 唯一保存。进展事件只包含阶段、分类、时间、必要 ID、缺失事实键和用户确认短摘要；不得通过 SQL 或日志补存消息正文。沟通成功的历史投递字段保持原样用于兼容，新流程不会再把“已发起沟通”写成 `applied`。

## 边界

扫描链路只读采集岗位。沟通链路只有在用户选择岗位、确认清单并明确点击开始后才会串行操作 BOSS；每项都要先核验岗位身份并在点击后验证结果。沟通成功只记为“已发起沟通 / 等待回复”，不代表已投递简历。项目不会后台定时沟通，也不会自动填写或发送模型生成的定制文案；粘贴 HR 消息、确认发送、确认面试和日历安排都由用户手工完成。

## 消息只读发现：运行与停止

入口为 `/messages?profileId=<个人资料 ID>`。该操作必须由用户启动，并独占 BOSS 页面租约 `discover-messages`；扫描、详情补读、批量沟通和消息发现不能同时运行。租约启动后持续续约，结束、失败或停止时释放。租约被占用或丢失时不访问页面、不继续点击，结果应显示为需要人工处理或相应固定错误码。

运行仅接受用户现有、唯一且固定标记为 `BOSS-COMMUNICATION` 的 BOSS 消息标签。首次只读扫描建立不可变 unread 队列，随后串行处理：每条会话在点击前验证行索引、未读状态和临时身份签名，点击后独立复核选中行、招聘方、岗位、会话和消息身份。守卫点击只允许点击目标未读会话行；禁止新建标签、导航、前台聚焦、读取 Cookie/localStorage、重放内部 API，以及聚焦、填充或发送聊天输入。

发现风险控制、登录失效、页面丢失、页面路径或 DOM 漂移、未读状态变化、目标身份不符或关联不唯一时，立刻安全停止本轮，不处理下一条。不要以修改选择器直接恢复：先用脱敏 fixture 做 fixture-first 校准，再在真实页面执行最小的只读发现验证。真实页面不得保存截图或聊天内容。

运行结果只暴露状态、计数、分类和安全摘要，不含 HR 文本、招聘方原文、预览、草稿、完整 URL 或 DOM 快照。可靠关联 `profile + job + thread` 后，草稿也只驻留内存，最多两条；缺失关联为 `needs_user_action`。用户手动发送后点击“已手动发送”；“停止”中断后续队列，“忽略/放弃”删除内存草稿，启动新一轮、30 分钟到期和服务重启同样删除草稿。面试邀请仅记录提醒，绝不生成确认安排。