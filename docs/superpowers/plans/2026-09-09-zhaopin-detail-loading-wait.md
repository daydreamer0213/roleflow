...
- [x] 最终冻结SHA `8b0d719b9b63500ecc088192c418cf9f0bf0a3c4` 严格完整npm test为157/157、退出0、起止SHA相同且树干净；04:17日志在04:33复核结束。1690622取消记录不作为通过证据。
- [x] 真实长期空白等待约120秒后准确中断、保留63岗位/62完整/1短JD待补；超时岗位尚未入库，不能把待补1项误认成超时卡。用户报告切前台仍空白、手动刷新后才恢复；04:44正常UI一次继续同一run，已保存新完整详情，无自动前台恢复。
- [x] 04:49只读数据库核对：本次120秒超时的原岗位已在04:45正常读取并保存1145字符正文；岗位及批次观察重复分组均为0。这仅证明用户刷新后本次原岗位恢复，不代替更早另一空白岗位或整轮通过。
- [ ] 后续完整扫描/分析仍待真实结果；不以新页面正常或恢复状态代替整轮通过。正常运行维持后台，不增加自动刷新、重点击或降低质量/覆盖。

04:59验收补充：本次继续在第20卡再次超时，中断保留70新岗位/69完整/1既有短JD；不重复按继续。实际站点客户端默认3秒超时，详情接口复用该客户端；请求结束后网站loading=false、详情为空，解释了为何单纯等待可能没有效果。具体网络失败原因尚未捕获，不认定后台是根因。本等待修复与复审不重开；单次后台只读重读属于下一项待确认的恢复策略，不在本计划内偷偷启用。3项定向复验于04:53完成，仍无外部发送。
