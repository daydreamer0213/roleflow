...
- [x] **Step 1: Write and observe failing behavior tests.** Recovery note: the original implementation's RED record was not retained; existing regressions were freshly re-run, and the subsequent skeleton/risk fixes have observed RED/GREEN. This checkbox records current regression completion, not recovery of the missing historical evidence. Extend live-structure synthetic reader fixture with `.im-chat-header__detail`, preserving parameterized IM URL and header/login regressions. Add standalone detail fixture generated from observed selectors with synthetic names/text. Test:
...
- [x] **Step 3: Verify targeted regressions and integration.**
...
- [x] **Step 4: Commit and independent review.** Implementation `ab7a246`, reviewed fix `5664310`; initial seven focused checks and three fix-covering checks passed. No remaining Critical/Important. Commit only owned code/tests. Record RED/GREEN, covering command/output, commit SHA and outstanding live assumptions. Main then restarts the isolated app and performs real message -> detail -> real model -> saved draft from product UI, stopping before reply send. This real journey remains pending with the other phase work.
