...
- [x] **Step 1: Add integration tests with real temporary SQLite and fake reader/model boundaries, observe RED.**
...
- [x] **Step 2: Generalize the existing pipeline only at its source-dependent seams.**
...
- [x] **Step 3: Run focused GREEN and existing message/learning/storage contracts, diff-check and commit owned files.** Main controls final full gate. Report exact actual table fixture used, no prose/source-text assertions.

### Task 4: Unified manual discovery action and inbox

**Files (ownership):** Modify `src/dashboard/message_discovery_controller.js`, `src/dashboard/message_discovery_view.js`, `src/dashboard/server.js`, `src/dashboard/ui/navigation.js` and only necessary existing dashboard CSS; tests `tests/dashboard_message_discovery_smoke.js`, `tests/dashboard_message_reply_send_smoke.js`, `tests/dashboard_communication_profile_smoke.js`, `tests/dashboard_shell_smoke.js`, `tests/dashboard_zhaopin_smoke.js`, `tests/zhaopin_message_reader_smoke.js`, `tests/zhaopin_message_discovery_smoke.js` or one focused local journey registered in `tests/run_all.js`. No new framework or UI dependency.
