...
Inject an analytics reader that throws once. Assert that /queue returns HTTP 200, preserves its existing queue heading, keeps the analytics panel at its fixed position, isolates that panel, and asserts its content is exactly and only the Chinese text `统计暂不可用` with no extra non-exception text or exception details. It must also emit exactly one `outcome_analytics_render_failed` warning containing only the fixed error code `OUTCOME_ANALYTICS_UNAVAILABLE`.
