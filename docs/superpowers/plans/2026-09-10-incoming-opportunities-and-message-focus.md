...
**验证检查点（2026-09-10）：** 实现与独立复核完成，现有资料四种宽度页面验收通过；冻结 `9e8c655f67d37f8163470f3074fcb18a3f89b333` 的严格完整检查 160/160、退出 0、起止 SHA 一致且工作树干净。最终文档 SHA 复验与远端核对另存仓库外 `incoming-focus-push-receipt.json`，不以复选框替代回执。见[验收报告](../reports/2026-09-10-incoming-opportunities-message-focus-acceptance.md)。
...
**Steps:**
- [x] 对真实持久层写手算 fixture：无 outbound + 未满 48 小时的请求立即进入；重复上下文合一；未关联正文/请求包含、空白排除；跨平台隔离；多 profile 隔离；关联前后不重复；已处理/历史分类仍可核对。记录行为 RED。
- [x] 实现最小聚合与体检展示/精确明细、必要兼容现有测试，记录 GREEN。
- [x] 运行定向回归、自审并提交本任务显式文件；报告接口和潜在无法匹配的历史数据，不隐瞒。
- [x] 独立任务复审（符合设计和代码质量）关闭重要问题。
...
**Steps:**
- [x] 阅读 Task 1 接口报告与现有保存/复制/批次脚本，扩展真实浏览器 fixture 写 RED：首屏无预选批次、点请求统计到对应联系、窄窗进退、备用稿、切换保存失败保留。
- [x] 最小改模板、客户端状态和 CSS；运行对应 GREEN、消息控制/学习/发送安全回归。
- [x] 自审并提交本任务显式文件，报告测试与未验证项。
- [x] 独立任务复审，关闭重要问题。
...
- [x] 用临时 DB + 本地浏览器走“统计 → 对应消息 → 编辑/保存 → 切换/返回”，不发送。1440px 和窄窗截图复核首屏，确认不是只满足 HTML 断言。
- [x] 确认 8788 属于本任务且空闲后重启该服务；现有用户 DB 只读核对实际计数，不注入样本、不访问平台，不清除结果。
- [x] 做一次整个本次功能差异的独立复审，处理有依据的重要发现。
- [x] 冻结干净代码 SHA，从头 `npm test`，记录实际总数、退出码和起止 SHA，不在运行中编辑仓库。
- [x] 更新 NEXT_PHASE、PROJECT_HANDOFF、本计划和验收报告；最终文档 SHA 再做相称定向回归与 diff 检查。
- [x] 仅推送 `codex/zhaopin-readonly`，核对远端相等、main 未改变；给用户简短验收入口和未验证边界。

收口回执：验收文档提交 `f7c8de48a5a0e22128a66a8743a739ac19aa9df4` 上三项定向回归与差异检查通过，源码及测试与完整门禁 SHA 一致；已推送并核对远端相等，main 保持 `15dc54867921ab3f621aa14a1070f7df180a3954`。本次仅记录复选框收口，最终记录提交的复验与远端状态仍以仓库外回执为准。
