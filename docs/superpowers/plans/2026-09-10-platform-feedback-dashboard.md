...
- [x] 增加真实SQLite回归并先运行观察失败：两个平台各20岗位不能合成40样本诊断；早回复计数立即出现；只改BOSS后智联旧20和新1合为21；共享调整各自重新计数，累计保留。
  ```js
  ...
  ```
- [x] 在策略快照存储验证过的`platformScope`。历史查询`limit:null`不截断；entries带真实jobs.source。用各平台有效修订ID集合筛选当前/前一组，保留物理ID及原事件。
  ```js
  ...
  ```
- [x] 从各平台全龄投影构建计数；诊断分别使用原成熟规则，智联不推断阅读能力；简历优化保留分平台结论。补测跨方案/候选人累计隔离、旧快照、迟到反馈、无效范围和重复提交。
- [x] 运行`node tests/funnel_platform_feedback_smoke.js`及既有funnel_diagnosis、strategy_round_store、message_linkage、zhaopin_communication_storage检查并提交。
...
- [x] 替换旧展示断言并先运行失败，验证当前与累计不同数值、2/5=40%早回复、零分母横线、未核验消息不混计；HTTP范围/平台验证及表单并发不回归。
  ```js
  assert.match(currentHtml, /2.*40%/);
  assert.match(lifetimeHtml, /累计记录/);
  assert.equal(invalidScope.status, 400);
  ```
- [x] 主页面改为范围切换+平台表+一条建议，其他消息简短入口，调整表单放details；删除旧轮次、证据尺和长说明对应渲染。只有reply显示百分比，所有文本转义。
- [x] 新增局部CSS沿用现有tokens，390像素下五列全部可见，页面及表格不横向溢出；键盘焦点及原生表单验证保留。
- [x] 执行真实服务+合成SQLite浏览器旅程，覆盖切换、折叠/提交平台范围和390/1280布局；不在用户DB提交策略修改。提交。

## Task 3: 验收收口与分支保存

Files: 本设计/计划、`docs/PROJECT_HANDOFF.md`, `docs/NEXT_PHASE.md`, 新验收报告。

- [x] 一次限定只读独立复审，检查统计范围、计数分母、策略继承和用户可见信息；修复实际问题并回归。
- [x] 核验仅拥有的8788服务身份/无运行任务，隐藏重启，真实本地页面复查两种范围与窄屏；不动安装版或外部平台。
- [x] 冻结源码提交，运行仓库外`run-frozen-gate.ps1`获得新鲜完整`npm test`实际总数和起止SHA；不在运行中改文件。
- [x] 更新验收记录和下一入口，检查差异并保留仅推送的交付边界。文档提交后的最终验证、SHA及推送结果由仓库外 `platform-feedback-push-receipt.json` 登记，不以本复选框预先认定推送完成。

## 验收结果

- 冻结 `b4b6b722f8ee6fcc0e07fb0f4bd1a50ffe31d388` 新鲜完整 `npm test`：159/159，退出0，起止SHA一致且树干净。
- 限定独立复审发现的三项问题已关闭：面试安排建议衔接、等待回复入口、未知旧基线被误当0%的比较。比较须两边各自拥有达到门槛的成熟且回复状态明确的记录。
- 本地8788两种范围、1440/390布局只读验收完成；没有在用户数据中写入虚假方案调整，没有访问真实招聘平台。
- 两次未完成门禁与Windows启动组计时依据如实保留在[验收报告](../reports/2026-09-10-platform-feedback-dashboard-acceptance.md)。
