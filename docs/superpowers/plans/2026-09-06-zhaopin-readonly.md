...
- Modify: `src/storage/job_store.js` 中方案重检查实际查询（保留客户公司及混合来源的原生条件边界）
...
- 方案保存/本轮准备调用的 `rescorePlanObservations` 必须读取客户公司再生成分析版本，不能因 SELECT 漏字段把客户公司清空。混合来源重新检查仍使用共享本地偏好，但不将 BOSS 原生采集条件套到智联观察记录上；补实际可达路径的定向回归。
- 小详情额度的部分目标恢复必须向后推进，不能每次重读同一前缀。复用现有 `getReusableDetail`：先现场核对 pane 真实岗位编号及完整内容，再复用同来源/候选人且内容未变的完整结果；新岗位或内容变化消耗本次新 JD 额度。`maxDetailTotal` 统计新补读/变化的 JD，未变结果记录为 reused；每次实际核验点击仍在动作前申请并计入站点访问额度、节奏与限制。达到新 JD 额度后不得继续点击下一岗。无新持久化游标，不把卡片签名当岗位身份，旧 BOSS 语义不变。
...
- Modify: `src/reports/render.js` 与必要的 CLI 报告调用参数（只修正来源、客户公司和不适用动作，不重设计报告）
...
- 共用运行状态栏 `ui/shell.js` / `assets/runtime.js` 也应遵守当前页面或冻结本轮来源：智联页面不能仍要求登录 BOSS 或让“重新检查工作区”强开 BOSS。保留原 BOSS 页面文案和恢复行为；不自动更换浏览器登录身份。
- 岗位结果有客户公司时，用紧凑文字区分“用人公司 / 发布方”，岗位机会描述优先基于已给出的客户公司，不能将外包招聘发布方当成实际用人公司；无客户公司时沿用原有显示。
- 本地生成报告也使用真实来源的标题和文件名前缀；空智联批次不能回退成 BOSS，混合来源报告按行标明来源。智联报告不展示招呼语复制等未启用动作，客户公司和发布方分列；保留旧 BOSS 报告调用兼容和现有版式。

- [x] 写 HTTP/本地浏览器失败检查：BOSS 默认为旧路径；选择智联打开带词页；保存和刷新后条件保留；启动请求携带来源；结果可见来源；无智联发送按钮；伪造批次请求被拒绝。
- [x] 沿用现有视觉样式和原生 select，不重做布局、不增加大面积说明；新增错误显示发生位置及下一操作，不只显示编号。
- [x] 本地页面完成一次合成数据完整流程与暂停恢复验证，检查桌面无溢出/页面错误/外部请求；记录真实平台未验收项。实现 `27b08f7`；精确提交的真实 HTTP/CLI/SQLite + headless Edge 本地旅程使用合成平台和分析器，暂停并重开服务/数据库后续跑同一本轮，3 条完整分析且总调用 3 次。证据见同日实现报告；不代表实站/真实模型验收。
...
- [x] 运行新鲜完整 `npm test`、危险夹具扫描、`git diff --check`；审查整个分支并修复当前可达问题。最终功能 `c27b2df` 的严格门禁 149/149、退出码 0，全部审查问题已关闭；详细日志与限制见实现报告。
- [x] 提交功能与最终文档，精确文档提交 `b28e9cc0a0075d060424ca0c1b79edb53e531d55` 再跑 12 项风险相关回归，全部通过。后续只更新验收记录，交付 SHA 与再次复验见外部 `final-verification.json`；不上线、不发布，后续由用户验收页面并单独授权集成。
