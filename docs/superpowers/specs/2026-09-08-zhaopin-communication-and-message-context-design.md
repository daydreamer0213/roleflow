...
最小产品入口复用已有独立沟通清单：智联分析完成后从岗位结果“选择岗位打招呼”，不重写已完成的只读扫描轮次或为旧轮次追补沟通目标。清单冻结单一来源和每个真实岗位；沟通中心必须能在刷新、重启或再次进入后找到独立批次，不要求用户记住地址。搜索页仅作实际网页按钮的工作页面，找回目标所需条件来自批次快照；用户当前搜索条件的变化不改写授权目标。
