...
日期：2026-08-12  
状态：已由 `2026-08-12-boss-explicit-dual-detail-mode-design.md` 收编并修订；本文仅保留早期探针和设计证据

> 2026-08-12 更新：受控前台监控确认 `Page.bringToFront` 会在当前环境把 Windows 前台切到 Edge。右栏链路仍保留为默认可信模式；同源 API 现按新的显式双模式设计进入独立静默校准，仍不得自动替换或回退。
