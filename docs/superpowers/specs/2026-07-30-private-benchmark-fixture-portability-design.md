...
同时继续验证画像确认链、匹配卡确认链、简历和身份清单哈希、模型身份哈希、确认 ID、确认时间及源产品提交。v3 证明只记录确认 ID 的 SHA-256，不记录原始确认 ID；旧 v1/v2 字段保持原样以兼容已有证明。
...
`proofSha256` 是防止误改和不完整改写的自校验，不是外部签名。延续既有私有验收威胁模型：本机管理员恶意重写完整证明、目标 manifest 和全部相关哈希不在防护范围内，因此不新增同目录 HMAC 或伪造不可实现的“本机管理员不可重写”保证。必须防住的是 runner 自动降级、仅改版本/字段的不完整改写、文件变更和提交/blob 不一致。
...
- 证明生成后篡改目标岗位、目标标签、目标提交、自哈希或 v3 确认 ID 哈希均在模型初始化前失败；
