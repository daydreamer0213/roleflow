...
| 5 | `contract05Lifecycle` asserts the complete scan-run shape; begin observes `BEGIN IMMEDIATE`/`COMMIT`; heartbeat persists; finish synchronizes the batch; equal terminal repetition is accepted and a different terminal state returns `SCAN_RUN_ALREADY_FINISHED`. Its direct `claimScanRun` success path asserts run/batch/lease/owner/process fields and batch sync with `BEGIN IMMEDIATE`/`COMMIT`; table-driven wrong-owner/already-claimed/expired-or-missing-lease/site-or-plan-mismatch failures assert coded errors, unchanged run/batch/lease snapshots, and `ROLLBACK`. |
| 6 | `contract06ProcessExit` asserts nonzero exit becomes `failed` with process fields, SIGTERM becomes `interrupted` with `processSignal`, zero exit follows completed/partial/failed target summaries, and a running rebound preserves its batch. A trigger after terminal batch synchronization must hit, produces `BEGIN IMMEDIATE`/`ROLLBACK`, and restores complete run/batch/target snapshots. |
| 7 | `contract07Orphans` asserts a fresh run remains running, a stale run with an active matching lease remains running, an expired matching lease is removed while its run is interrupted, a stale unprotected single run synchronizes its running batch to `interrupted`, a mismatched-owner active lease is retained, and an old run interrupted beside a running rebound leaves the shared batch running. |
...
| 9 | `contract09TargetHistoryAndSummary` asserts empty running summary, complete target-result row shape, repeated target attempts, latest-per-target ordering, completed/failed-derived partial aggregate counts and job total, plus exact missing-`batchId` and missing-`targetKey` messages. |
| 10 | `contract10RuntimeAndAccess` asserts complete runtime/access row shapes, runtime upsert/read/clear, exact missing-site/runtime and missing-site/action access messages, access payload normalization, site/action/since filtering, ascending order, limit, and a 10,001-row fixture capped at 10,000 results. |
...
| 12 | `contract12ReuseRefreshAndCatalog` asserts profile/site isolation, latest reusable observation, 120/119 description behavior, 3-day activity freshness, default/1-day/30-day max-age clamping, refresh numbering/reverse order/limit/error fields, complete catalog shape, conflict update, exact missing-site catalog message, and invalid-JSON fallback. |
| 13 | `contract13FacadeConsumers` executes the existing focused CLI, site-budget, workflow, workflow-health, dashboard scan-lifecycle, and storage-migration smoke tests as child processes and asserts each exits zero; it does not use source-text matching as consumer evidence. |
