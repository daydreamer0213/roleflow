...
- `codex/message-discovery-job-analysis` 已完成纯文字消息语义分流、岗位分析主视图、重试状态分离和真实积分影子报告。结构化消息卡片仍停在证据门：需要一次获授权的真实 BOSS 低频只读取证后，才能实现简历投递窗口等人工操作分流。
...
- 本次消息发现与岗位分析验证只使用 Mock、保存 fixture、假浏览器、临时数据库和冻结 Gate D 文件。没有访问真实 BOSS、没有调用真实语义模型，也没有执行真实发送，因此结构化消息卡片 DOM、真实消息语义准确率和重复模型稳定性仍是未验证前提。
