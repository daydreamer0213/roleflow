...
- 当前候选的完整离线门禁：**已通过**。2026-08-25 在实现提交 `508781d77fc62943825e12608f3cc230d60e21ca` 上运行 `npm test`，退出码为 0，107/107 项通过，原始末行为 `All 107 offline checks passed.`。
- 当前候选已完成本机安装、启动、专用 Edge 工作区自愈和两轮真实 BOSS 只读扫描验收；没有执行真实沟通、投递或申请。
...
- 当前完整安装包位于 `D:\DevData\RoleFlow-installer\acceptance-508781d\output\RoleFlow-Setup-1.0.0.exe`，SHA-256 为 `4b032feabd0a8f26390dfdd1e6253a3a34b13de70f278c97aefb5f614be795fb`。它是本机验收候选，不是已发布资产。
...
- 已知未完成项：岗位在本轮内首次模型分析失败、随后通过队列重试成功时，本轮进度仍正确保留首次失败历史，但流程体检仍可能把已解决岗位显示为“尚未完成语义分析”。下一步应区分“本轮历史失败”和“当前未解决状态”，不能简单抹掉历史失败。
- 交付必须列出修改文件、验证命令与结果、未验证前提、真实平台读写情况、stage 路径和提交哈希。
- 不自动推送、合并或发布。安装、启动 Edge 和真实 BOSS 只读验收本轮已由用户持续授权；任何真实沟通、投递或申请仍需对当前不可变目标明确授权。
