...
## 消息发现与求职进展集成分支（2026-08-06）

当前协作分支：`codex/integrate-candidate-progress-message-flow`，基线 `origin/main@c013140b`，不合并 main，不合并岗位匹配分支。

已完成：

- 求职进展卡：唯一 `profileId+jobId` 卡，9 个阶段，事件幂等，事务回滚。
- 沟通成功语义：不再写 `applicationStatus=applied`，改为 `waiting_reply` 进展卡；失败路径、校准门和 BOSS 核验保持不变。
- 只读消息页读取器：公司名、稳定会话键、预览类型、非文本内容识别；真实 DOM 校准未完成（当时无 `BOSS-COMMUNICATION` 消息页），已记录 fixture 回退。
- 预览基线：首次观察只建基线，未读与预览变化双通道排队；只存摘要哈希。
- 消息组：连续 HR 文本最多 5 条一组，事务级幂等落库；事实有效期与稳定作用域。
- 回复契约：完整覆盖、事实新鲜度、面试邀约无草稿、内存草稿清理。
- 控制台：先租约后建浏览器、草稿最多 2 条、30 分钟清理、手动发送/放弃清理、页面轮询不触碰 BOSS。
