...
async function main() {
  ...
  try {
    ...
    const zlCard=page.locator('[data-message-detail-panel].message-result[data-platform="zhaopin"]').filter({has:page.locator('[data-draft-text]')});assert.equal(await zlCard.locator('[data-send-single], [data-send-select], [data-sent-draft]').count(),0);assert.equal(await zlCard.locator('[data-copy-draft]').count(),1);
    ...
    let saveMode='fail',deferredSaveStarted=()=>{},releaseDeferredSave=()=>{};await page.route('**/api/message-reply-draft',async route=>{if(saveMode==='fail')return route.fulfill({status:500,contentType:'application/json',body:'{"errorCode":"SAVE_FAILED"}'});if(saveMode==='defer-then-fail'){saveMode='fail';deferredSaveStarted();await new Promise(resolve=>{releaseDeferredSave=resolve;});}return route.continue();});await field.fill('保存失败时保留的回答');await zhaopinPendingRow.click();await page.waitForFunction(()=>document.querySelector('[data-discovery-feedback]').textContent.includes('保存'));assert.equal(await filter.inputValue(),'zhaopin');assert.equal(await field.inputValue(),'保存失败时保留的回答');assert.equal(await zlCard.isVisible(),true);assert.equal(await zlRow.locator('[data-message-view]').isChecked(),true);saveMode='pass';
    let markDeferredSaveStarted;const deferredSavePending=new Promise(resolve=>{markDeferredSaveStarted=resolve;});deferredSaveStarted=markDeferredSaveStarted;saveMode='defer-then-fail';await field.fill('延迟保存的第一版');await zhaopinPendingRow.click();await deferredSavePending;await field.fill('等待期间继续输入的第二版');releaseDeferredSave();await page.waitForFunction(()=>document.querySelector('[data-discovery-feedback]').textContent.includes('保存'));assert.equal(await field.inputValue(),'等待期间继续输入的第二版');assert.equal(await zlCard.isVisible(),true,'a failed edit made during an in-flight save must keep the original message visible');assert.equal(await zlRow.locator('[data-message-view]').isChecked(),true);saveMode='pass';
