...
async function main() {
  ...
  try {
    ...
    let failSave=true;await page.route('**/api/message-reply-draft',route=>failSave?route.fulfill({status:500,contentType:'application/json',body:'{"errorCode":"SAVE_FAILED"}'}):route.continue());await field.fill('保存失败时保留的回答');await zhaopinPendingRow.click();await page.waitForFunction(()=>document.querySelector('[data-discovery-feedback]').textContent.includes('保存'));assert.equal(await filter.inputValue(),'zhaopin');assert.equal(await field.inputValue(),'保存失败时保留的回答');assert.equal(await zlCard.isVisible(),true);assert.equal(await zlRow.locator('[data-message-view]').isChecked(),true);failSave=false;
