...
server.listen(0, "127.0.0.1", async () => {
  ...
  try {
    ...
    const responsibilityConfirmationCalls = [];
    let responsibilityConfirmationAttempt = 0;
    const responsibilityConfirmationAdapter = new OpenAICompatibleAdapter({
      baseUrl,
      apiKey: "test-key",
      model: "test"
    });
    responsibilityConfirmationAdapter.chatJson = async (_prompt, modelInput, { kind }) => {
      responsibilityConfirmationCalls.push({ kind, modelInput });
      if (kind === "matchResponsibilities") {
        responsibilityConfirmationAttempt += 1;
        return responsibilityConfirmationAttempt === 1
          ? {
            selectedTrackId: "T1",
            matches: [{
              id: "D1",
              state: "transferable",
              resumeEvidence: "简历：已有相邻职责交付事实"
            }, {
              id: "D2",
              state: "missing",
              gapDimension: "main_action",
              resumeEvidence: "简历：首次判断没有找到直接事实"
            }]
          }
          : {
            selectedTrackId: "T1",
            matches: [{
              ...
              id: "D2",
              state: "transferable",
              resumeEvidence: "简历：二次窄确认找到可迁移的具体项目事实"
            }]
          };
      }
      return {
    ...
    const responsibilityConfirmationInput = {
      ...splitInput,
      jobUnderstanding: {
        ...splitInput.jobUnderstanding,
        hiringTracks: [
          ...splitInput.jobUnderstanding.hiringTracks,
          {
            id: "T2",
            label: "另一个招聘方向",
            roleSummary: "交付另一个招聘方向",
            responsibilityEvidence: [
              "JD：负责另一个方向职责一",
              "JD：负责另一个方向职责二"
            ]
    ...
    const responsibilityConfirmedDecision = await responsibilityConfirmationAdapter.matchJob(
      responsibilityConfirmationInput
    );
    assert.deepStrictEqual(
      responsibilityConfirmationCalls.map((call) => call.kind),
      ["matchResponsibilities", "matchResponsibilities", "matchRequirements"],
      "首次职责结果包含 missing 时必须追加一次窄职责确认，再运行要求阶段"
    );
    assert.deepStrictEqual(
      responsibilityConfirmationCalls[1].modelInput.hiringTracks.map((track) => track.id),
      ["T1"],
      "职责确认输入必须只保留首次验证通过的招聘方向"
    );
    assert.strictEqual(
      responsibilityConfirmedDecision.responsibilityMatches.find((item) => item.id === "D2").state,
      "transferable",
      "任一次职责确认给出有效正向证据时必须消除单次 missing 波动"
    );

    const missingWorkObject = normalizeResponsibilityOutput({
      selectedTrackId: "T1",
      matches: [{
        id: "D1",
        state: "missing",
        gapDimension: "work_object",
        resumeEvidence: "简历：首次确认工作对象不兼容"
      }]
    }, splitInput.jobUnderstanding);
    const missingDeliverable = normalizeResponsibilityOutput({
      selectedTrackId: "T1",
      matches: [{
        id: "D1",
        state: "missing",
        gapDimension: "deliverable",
        resumeEvidence: "简历：二次确认交付结果不兼容"
      }]
    }, splitInput.jobUnderstanding);
    assert.deepStrictEqual(
      reconcileResponsibilityOutputs(missingWorkObject, missingDeliverable).matches,
      [],
      "两次 missing 的 gapDimension 不一致时必须降为省略式 unknown"
    );

    const confirmationRepairCalls = [];
    let confirmationRepairResponsibilityAttempt = 0;
    let confirmationRepairRequirementAttempt = 0;
    const confirmationRepairAdapter = new OpenAICompatibleAdapter({
      baseUrl,
      apiKey: "test-key",
      model: "test"
    });
    confirmationRepairAdapter.chatJson = async (_prompt, modelInput, { kind }) => {
      confirmationRepairCalls.push({
        kind,
        repair: Boolean(modelInput.contractRepair)
      });
      if (kind === "matchResponsibilities") {
        confirmationRepairResponsibilityAttempt += 1;
        if (confirmationRepairResponsibilityAttempt === 1) {
          return {
            selectedTrackId: "T9",
            matches: []
          };
        }
        if (confirmationRepairResponsibilityAttempt === 3) {
          return {
            selectedTrackId: "T2",
            matches: [{
              id: "D1",
              state: "missing",
              gapDimension: "main_action",
              resumeEvidence: "简历：另一个招聘方向的职责事实"
            }]
          };
        }
        return {
          ...
          matches: [{
            ...
            state: "missing",
            gapDimension: "main_action",
            resumeEvidence: "简历：所选招聘方向的职责事实"
          }]
        };
      }
      confirmationRepairRequirementAttempt += 1;
      return confirmationRepairRequirementAttempt === 1
        ? {
          matches: [{
            id: "R9",
            state: "matched",
            resumeEvidence: "简历：错误要求 ID"
          }],
          eligibility: []
        }
        : {
    ...
    await confirmationRepairAdapter.matchJob(responsibilityConfirmationInput);
    assert.deepStrictEqual(
      confirmationRepairCalls,
      [
        { kind: "matchResponsibilities", repair: false },
        { kind: "matchResponsibilities", repair: true },
        { kind: "matchResponsibilities", repair: false },
        { kind: "matchResponsibilities", repair: true },
        { kind: "matchRequirements", repair: false },
        { kind: "matchRequirements", repair: true }
      ],
      "职责确认路径的 chatJson 语义尝试必须硬限制为三阶段各一次初始和一次修复"
    );

    const confirmationFailureCalls = [];
    let confirmationFailureResponsibilityAttempt = 0;
    const confirmationFailureAdapter = new OpenAICompatibleAdapter({
      baseUrl,
      apiKey: "test-key",
      model: "test"
    });
    confirmationFailureAdapter.chatJson = async (_prompt, _modelInput, { kind }) => {
      confirmationFailureCalls.push(kind);
      if (kind !== "matchResponsibilities") {
        throw new Error("requirements must not run after confirmation repair fails");
      }
      confirmationFailureResponsibilityAttempt += 1;
      return confirmationFailureResponsibilityAttempt === 1
        ? {
          selectedTrackId: "T1",
          matches: [{
            id: "D1",
        ...
        : {
          ...
          matches: [{
            ...
            state: "missing",
            gapDimension: "main_action",
            resumeEvidence: "简历：错误招聘方向的职责事实"
          }]
        };
    };
    await assert.rejects(
      () => confirmationFailureAdapter.matchJob(responsibilityConfirmationInput),
      (error) => error.code === "MODEL_CONTRACT_INVALID"
        && error.modelStage === "matchResponsibilities"
        && error.modelPhase === "contract_repair"
    );
    assert.deepStrictEqual(
      confirmationFailureCalls,
      ["matchResponsibilities", "matchResponsibilities", "matchResponsibilities"],
      "职责确认及其一次修复均冲突后必须失败关闭，且不得运行要求阶段"
    );
