...
6. `foundation`、`central` 与 `indispensable` 相互独立。模型负责普通技能、
   ...
   Java、PMP、AI 等领域词表。明确软条件归一为非硬门槛；能够可靠归属到单一
   要求且不混有软条件的明确硬性/否决措辞归一为硬门槛。含糊语义和无法可靠
   拆分的复杂同句有限度保留模型判断。模型布尔值可以参与核心排序，但核心
   `skip` 必须再有 JD 明确硬边界和简历明确冲突。
   `responsibility_sprawl` 只作为 JD 职责偏宽的岗位质量提示，不单独触发
   `review` / `skip`；模型给出的岗位质量 `caution` 仍可保留轻度提醒，其他
   中高等级语义风险也继续使用原有护栏。
   `understandJob` / `matchJob` 固定使用 `temperature=0` 来减少同输入采样
   漂移，不增加调用次数；画像、沟通等其他生成任务仍使用原配置。
...
- Added `adjacent_misaligned` as an evidence-bearing review-only state for neighboring layers/channels within the same primary artifact class and professional delivery lifecycle. It reuses the existing six-key contract and, absent another hard gate, derives review/backup; different artifact classes or specialist disciplines remain hard `misaligned`. Match cache is `match-decision-v37`.
