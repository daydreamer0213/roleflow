...
const PAGE_SPECS = Object.freeze([
  { id: "today-ready", family: "today", state: "ready", primarySelector: "[data-today-primary]", path: ({ planId }) => `/plan?planId=${planId}`, interaction: "none" },
...
async function main() {
  ...
  if (process.env.ROLEFLOW_REQUIRE_PLAYWRIGHT !== "1") {
    throw new Error("ROLEFLOW_REQUIRE_PLAYWRIGHT=1 is required for the strict dashboard acceptance.");
  }
  let chromium;
  try {
    ({ chromium } = require("playwright"));
  } catch {
    throw new Error("Playwright is unavailable. Set NODE_PATH to the existing Codex runtime node_modules directory; do not install dependencies.");
  }
  validateOptions(options);
  const targetCleanAtStart = gitStatus(options.targetRoot) === "";
  if (options.strict && !targetCleanAtStart) throw new Error("Strict dashboard acceptance requires a clean target revision before evidence generation.");
  ...
  const result = {
    ...
    viewports: VIEWPORTS,
    pageSpecs: PAGE_SPECS.map(({ id, family, state, interaction }) => ({ id, family, state, interaction })),
    pages: [],
    errors: []
  };
  ...
  try {
    ...
    for (const spec of PAGE_SPECS) {
      for (const viewport of VIEWPORTS) {
        ...
        if (options.strict) assertStrictPage(page);
      }
    }
  } catch (error) {
...
function seedFixture({ storage, matchingCardFromProfile, createCommunicationBatch, db }) {
  const profile = {
    candidate: { name: "Dashboard acceptance fixture", city: "Shanghai", targetTitles: ["AI application engineer"], expectedSalary: "15-25K" },
    education: [], experiences: [], skills: [{ name: "Python", evidence: ["offline fixture"] }],
    projects: [{ name: "RAG fixture", canSay: ["RAG"] }], credentials: [], strengths: []
  };
  const saved = storage.saveProfileAnalysis(db, {
    profile,
    document: { originalFileName: "dashboard-wave2-fixture.txt", format: "text", contentHash: "dashboard-wave2-offline-v1", text: "Python RAG offline dashboard acceptance fixture. ".repeat(16), diagnostics: {} },
    searchPlan: { name: "Dashboard acceptance plan", cities: ["Shanghai"], directions: ["AI application"], keywords: [{ word: "RAG", priority: "A", reason: "fixture" }], experience: ["1-3 years"], jobTypes: ["full time"], degrees: [], salary: { minK: 15, maxK: 25 }, bossActiveDays: 3, platform: { site: "boss" } }
  });
  ...
  const jobId = storage.upsertJob(db, {
    source: "boss", sourceId: "dashboard-wave2-primary", keyword: "RAG", title: "Offline dashboard role", company: "Fixture Co", location: "Shanghai", salary: "20-25K", experience: "1-3 years", education: "Bachelor", bossActiveText: "Active today", bossActiveDays: 0,
    url: "https://www.zhipin.com/job_detail/dashboard-wave2-primary.html", tags: ["Python", "RAG"], description: "Complete Python RAG job description used only by the local dashboard acceptance fixture. ".repeat(6), score: 20, level: "priority", matches: ["Python", "RAG"], risks: [], qualityTags: [],
    analysis: { semanticStatus: "complete", recommendation: "primary", recommendationSchemaVersion: 2, fitLevel: "fit", confidence: 0.9, fitReasons: ["Python and RAG fixture evidence"], evidence: { jd: ["Python RAG"], resume: ["Python RAG"] } }
  }, batchId);
...
async function auditPage({ browser, baseUrl, spec, fixture, viewport, outputDir, label, revision }) {
  ...
  try {
    ...
    const target = spec.primarySelector
    ...
    return { id: spec.id, family: spec.family, state: spec.state, revision, route, viewport, screenshot, audit, interaction, errors };
  } finally {
    await context.close();
  }
}
...
async function runClientOnlyInteraction(page, kind) {
  if (kind === "stop-preview-cancel") {
    const preview = page.locator('[data-action="stop-preview"]').first();
    const cancel = page.locator('[data-action="stop-cancel"]').first();
    if (!await preview.count() || !await cancel.count()) return { kind, attempted: false, passed: false, result: { reason: "control missing" } };
    await preview.click();
    const visible = await page.locator("[data-stop-confirmation]").isVisible();
    await cancel.click();
    const hidden = await page.locator("[data-stop-confirmation]").isHidden();
    return { kind, attempted: true, passed: visible && hidden, result: { visible, hidden } };
  }
  if (kind === "details-toggle") {
    const details = page.locator("main details").first();
    if (!await details.count()) return { kind, attempted: false, passed: false, result: { reason: "details missing" } };
    const before = await details.evaluate((element) => element.open);
    await details.locator("summary").click();
    const after = await details.evaluate((element) => element.open);
    return { kind, attempted: true, passed: before !== after, result: { before, after } };
  }
...
function pageAudit({ pageId = "", primarySelector = "" } = {}) {
  ...
  return {
    ...
    overflowElements,
    shell: {
      roleflowStylesheet: Boolean(document.querySelector('link[href="/assets/roleflow.css"]')),
      framePresent: Boolean(document.querySelector(".app-shell")),
      present: Boolean(document.querySelector('link[href="/assets/roleflow.css"]')),
      navigationPresent: Boolean(document.querySelector("nav")),
      mainPresent: Boolean(document.querySelector("main")),
      navigationMinTarget: navigationTargets.length ? Math.min(...navigationTargets.map((element) => element.getBoundingClientRect().height)) : 0,
      activeNavigation: [...document.querySelectorAll('nav a[aria-current="page"]')].map((element) => ({ text: text(element), href: element.getAttribute("href") })),
      activeNavigationCount: document.querySelectorAll('nav a[aria-current="page"]').length
    },
...
function assertStrictPage(page) {
  ...
  if (!shell.present) failures.push("missing shell");
  if (!shell.navigationPresent) failures.push("missing navigation");
  ...
  if (!page.interaction?.passed) failures.push("failed interaction");
...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (key === "--no-strict") { strict = false; continue; }
  ...
  return { help: false, strict, targetRoot: path.resolve(values.get("--target-root") || process.cwd()), label: values.get("--label") || "current", outputDir: path.resolve(values.get("--output-dir") || path.join(process.cwd(), ".runtime", "dashboard-wave2-evidence")), browserChannel: values.get("--browser-channel") || "msedge" };
}
