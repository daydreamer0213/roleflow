...
async function auditPage({ browser, baseUrl, spec, fixture, viewport, outputDir, label, revision }) {
  ...
  try {
    ...
    const target = spec.primarySelector ? page.locator(`${spec.primarySelector}:not([disabled])`).first() : page.locator("main a[href], main button:not([disabled]), main input:not([type=hidden]):not([disabled]), main select:not([disabled]), main summary").first();
