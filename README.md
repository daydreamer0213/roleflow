...
- 当前公开快照包含 42 组离线检查，覆盖扫描、存储、模型适配、工作流恢复、沟通执行和求职进展。
...
简历文件/文本
  ...
  -> 用户确认本轮沟通清单
  -> 串行沟通并核验结果
  -> 保存已发起沟通、等待回复、失效、待复核和未访问替补状态
  -> 用户按需粘贴 HR 消息，分类并生成事实约束草稿
  -> 用户复制、手动发送并确认求职进展
```
...
“已发起沟通”与“已投递简历”是两个独立阶段。每个候选人和岗位最多有一张进展卡，等待回复、需要处理、回复草稿、面试邀约、面试安排、简历投递、拒绝和关闭都保留为结构化事件。用户可以纠正阶段或重新开启机会，历史事件不会删除。

用户粘贴 HR 消息后，系统只在当前请求中使用原文。缺少 GAP、离职、到岗、项目上线等已确认事实时不会生成答案；有事实时最多提供两条草稿，由用户复制并在平台手动发送，再点击“已手动发送”。面试邀约不会生成建议回复，也不会访问或写入日历。
...
- HR 原话、生成回复正文、完整聊天和截图不写入数据库；进展事件只保存分类、阶段、必要标识、缺失事实键和用户确认短摘要。
...
沟通批次保存不可变的岗位、公司和链接快照。执行前必须由用户确认清单；执行器逐个打开岗位并校验身份，只有岗位一致且“立即沟通”状态明确时才允许单次点击。成功或平台已显示沟通时只建立“已发起沟通 / 等待回复”进展卡，不再写成“已投递简历”；岗位失效、身份不符、按钮不可用和结果不明确仍分别记录。
...
第一阶段的“开始只读发现”是用户主动启动的后台发现，不是自动回复：它不会填写 BOSS 输入框，也不会发送消息。打开本地地址 `/messages?profileId=<个人资料 ID>`，确认页面显示目标个人资料后，先在用户现有且唯一的固定 `BOSS-COMMUNICATION` 标签中打开 BOSS 消息页，再点击“开始只读发现”。

本轮流程为：扫描未读会话并固定队列 -> 一次只串行处理一条 -> 复核个人资料、岗位和会话身份 -> 在内存中分类并至多生成两条可复制草稿 -> 用户自行复制到 BOSS 并手动发送 -> 回到 RoleFlow 点击“已手动发送”。岗位、个人资料或会话无法可靠唯一关联时，状态为 `needs_user_action`，不生成草稿；原有“人工粘贴 HR 消息”入口始终可用。面试邀请只标记并提醒用户自行决定，不生成确认、接受、改期或安排面试的回复。

后台“点开会话”仅限用户现有的固定 `BOSS-COMMUNICATION` 标签内：对扫描时确认的未读会话行做身份复核后执行受保护点击。它不新建标签、不导航、不把标签带到前台、不聚焦页面，也绝不触碰输入框或发送按钮。发现登录失效、风险控制、页面漂移、租约丢失或身份不一致时会安全停止；DOM 变化时先使用脱敏 fixture 完成校准，再考虑真实页面的最小只读发现。

不会保存 HR 原话、回复正文、完整聊天、截图、Cookie、localStorage 或敏感 URL。草稿只存在 dashboard 进程内存中；用户放弃、启动新一轮、30 分钟到期或服务重启后都会清除。