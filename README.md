...
RoleFlow 每天最多由用户手动启动两轮相同任务。每轮都要经过清单确认才会串行点击沟通；不会后台定时运行，也不会绕过用户确认直接发送。
...
7. 在“今日求职任务”点击“执行一轮”。扫描和模型分析完成后检查默认勾选清单，确认后再开始串行沟通；当天需要时可再次执行同一流程。
...
每天成功沟通目标为 70，最多执行两轮，单轮目标不超过 40。第一轮通常目标 35；后续目标根据当天已成功数量动态计算。两轮共享每日 240 个右栏详情和 40 次页面导航预算，第一轮只能取得剩余预算的公平份额。每轮最多选择 3 个关键词，并优先使用当天尚未使用且历史有效率更高的关键词。
...
- 左栏明确硬排除后的唯一岗位按关键词优先级读取右栏；A/B 每词最多读取 45/30 个详情，一轮还受共享预算限制。达到上限或读取失败的岗位保留为待读详情。详情补读和活跃探针默认最多各 8 个。
...
- `docs/two-run-workflow-validation.md`：双轮流程、恢复方式和低流量真实验收清单。
...
当前代码和离线回归已启用校准后的沟通执行路径，但大批量真实页面效果仍需按 [双轮工作流验收说明](docs/two-run-workflow-validation.md) 从低流量开始验证。离线测试不会访问 BOSS，也不能替代真实页面验收。
